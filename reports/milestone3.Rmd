...
library(aweek)
library(kableExtra)
library(here) #install if you don't have
...
str(sim_pop)
head(sim_pop)
unique(sim_pop$cdc_week)

# rate per 100k
fun <- sim_pop %>%
  mutate(
    weekly_incidence_per100k = (new_infections / pop) * 100000,
    weekly_severe_per100k = (new_severe / pop) * 100000
  )

head(fun) %>% 
  select(county, age_cat, cdc_week, new_infections, pop, weekly_incidence_per100k)
```


Looking at whole California over time
- Aggregate by week
- All ages, all races, all sexes

Viewing the dataset you can see peak months are August, Sept, Oct
```{r fun part 1}
fun1 <- fun %>% 
  group_by(cdc_week) %>% 
  summarise(
  ...
  mutate(
    ...
    ci_severe100k = (total_severe / total_pop) *100000
  )
## Peak months are August, September, October
```

Look at age group over time
- there are differences in rates between age groups 
- At a quick glance severe CI is greater in 65+ most weeks
- to make it easier to see what's going on for total CI I'm going to graph it
```{r fun part 2}
fun2 <- fun %>% 
...
Looking at race/ethnicity over time
- graph shows there are differences in CI between race/ethnicity
- American Indian/Alaskan Native are observed to have the greatest peaks in CI
```{r fun part 3}
fun3 <- fun %>% 
